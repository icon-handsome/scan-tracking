	                                 ...
	                                 std::vector<cv::Point2f>  &results)
{
	...
	{
		std::cout << "¶ƒ????????: " << channels << "????????ß“?????..." << std::endl;
		return 1010;
	}
	...
	for (auto& cnt : contours)
	{
		...
		if (area > (config.min_area / pow(4, level_cnt)) && area < (config.max_area / pow(4, level_cnt)))
		{
			...
			if (perimeter < perim_t)            // ??ß≥??5???????????5/64 * 2 * 3.14 = 0.49
			{
				continue;
			}
...
{
	// 1. ??????????
	const int radius = 5;        // ???????????????ß≥? (2*radius + 1)
...
//	std::vector<float> edge_weights;   // ?õ•???????????????
//
...
//		if (abs(dist - expected_r) < 2.0f)       // ????????????ß≥??2??????
//		{
						                 ...
						                 int minPts)
{
	...
	float eps2 = eps * eps;
	int n = points.size();
	std::vector<int> labels(n, -1); // -1: ¶ƒ????, 0: ????, >0: ??ID
	int clusterId = 0;
                                ...
                                float minPercent)
{
	...
	if (valid_frame_points.size() < 3)
	{
		...
		track_pose_last_min_pose_points = 3;
			std::cout << "???????????????????????????¶À??: " << valid_frame_points.size() << std::endl;
			return 500;
	}
	track_pose_last_neighbor_filtered_count = static_cast<int>(valid_frame_points.size());
	std::cout << "????????????: " << valid_frame_points.size() << " / " << frame_3d_points.size() << std::endl;
	...
	cv::Mat                  Rt;                      // ??çﬁ????????»Œ
	std::vector<cv::Point3f> corres_template_pnts;    // ??????????ßµ????????????
	...
	if (filtered_frame_3d_points.size() < (size_t)min_pose_points)
	{
		...
		std::cout << "??????????????? ???????¶À??: " << filtered_frame_3d_points.size() << std::endl;
		return 500;
	}
	...
	{
		std::cout << "RANSAC??????????????????¶À??: " << best_inlier_count << std::endl;
		return 501;
	}

	Rt = best_Rt.clone();
	std::cout << " RANSAC Rt??from Template to Vision?? is: " << std::endl;
	...
	std::cout << "RANSAC????????? " << best_inlier_count << std::endl;
	...
	{
		std::cout << "????????????????????????¶À??: " << new_filtered_frame_3d_points.size() << std::endl;
		return 502;
	}
	...
	std::cout << std::fixed << std::setprecision(8);  // ?????? 8 ¶Àß≥??
	for (int i = 0; i < 4; i++)
	{
		for (int j = 0; j < 4; j++)
		{
